...
def play_single_game(args):
    """Play a single self-play game (for parallel execution)"""
    game_idx, board_size, model_state_dict, device_str, simulations = args
    
    # Set device for this process
    device = torch.device(device_str)
    
    # Create model and load state
    policy_value_net = PolicyValueNet(board_size, num_blocks=5)
    ...
    player = AlphaGoPlayer(policy_value_net, 
    ...
    while not game.game_over and move_count < max_moves:
        ...
        try:
            ...
            print(f"\nError in game {game_idx}, move {move_count}: {e}")
            move = None
        ...
        game_data.append([state_tensor.cpu(), policy_target, 0])  # Move tensors to CPU for storage
...
class SelfPlayTrainer:
    def __init__(self, board_size=9, num_blocks=5, learning_rate=1e-3, buffer_size=10000, num_workers=None):
    ...
    def collect_game_data_parallel(self, num_games=25, simulations=50):
        """Collect self-play game data using parallel processing"""
        print(f"\nCollecting {num_games} games using {self.num_workers} workers...")
        
        # Prepare arguments for parallel execution
        model_state_dict = self.policy_value_net.state_dict()
        
        # For MPS, we need to use CPU for multiprocessing and then move results back
        device_str = 'cpu' if self.device.type == 'mps' else str(self.device)
        
        game_args = [
            (i, self.board_size, model_state_dict, device_str, simulations)
        ...
        with ProcessPoolExecutor(max_workers=self.num_workers) as executor:
            # Submit all games
            futures = {executor.submit(play_single_game, args): i 
                      for i, args in enumerate(game_args)}
            
            # Process completed games with progress bar
            with tqdm(total=num_games, desc="Self-play games") as pbar:
                for future in as_completed(futures):
                    ...
                    try:
                        ...
                        pbar.update(1)
                        pbar.set_description(f"Self-play games (avg {total_moves/(game_idx+1):.1f} moves/game)")
        ...
        print(f"Average: {avg_moves:.1f} moves/game, {avg_time:.1f}s/game")
        
        return len(all_game_data)

    def collect_game_data(self, num_games=1):
        """Original sequential game collection (fallback)"""
        games_data = []
        ...
        for game_idx in games_pbar:
            ...
            while not game.game_over and move_count < max_moves:
                ...
                game_data.append([state_tensor, policy_target, 0])
    ...
    def run(self, num_iterations=100, num_games_per_iter=25, batch_size=64, 
        ...
        print(f"Parallel workers: {self.num_workers if use_parallel else 1}")
        print(f"{'='*60}\n")
        ...
        for iteration in range(num_iterations):
            ...
            if use_parallel and self.device.type != 'mps':  # MPS doesn't work well with multiprocessing
                positions_collected = self.collect_game_data_parallel(num_games_per_iter)
            else:
                if self.device.type == 'mps':
                    print("   (Using sequential collection for MPS device)")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num-workers', type=int, help='Number of parallel workers (default: number of CPU cores)')
    parser.add_argument('--no-parallel', action='store_true', help='Disable parallel game generation')
